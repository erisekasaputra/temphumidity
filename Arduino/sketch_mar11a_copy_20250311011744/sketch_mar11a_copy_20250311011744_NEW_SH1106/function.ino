void initOLED(){
  //if(!display.begin(SH110X_SWITCHCAPVCC, 0x3C)) { // Address 0x3D for 128x64
...
void connecting2WiFi(const char* SSID, const char* PASS) {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    ...
    dotCount++;
    if (dotCount > 20) {
      //display.fillRect(0, 40, 128, 8, SH110X_BLACK); // Hapus area titik
       display.fillCircle(display.width() / 2, display.height() / 2, 10, SH110X_BLACK);
      dotCount = 0;
    }
    Serial.print('.');
